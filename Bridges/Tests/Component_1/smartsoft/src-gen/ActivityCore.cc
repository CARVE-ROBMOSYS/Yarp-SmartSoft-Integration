...
void ActivityCore::notify_all_interaction_observers() {
	std::unique_lock<std::mutex> lock(interaction_observers_mutex);
	// try dynamically down-casting this class to the derived class 
	// (we can do it safely here as we exactly know the derived class)
	if(const Activity* activity = dynamic_cast<const Activity*>(this)) {
		for(auto it=interaction_observers.begin(); it!=interaction_observers.end(); it++) {
			(*it)->on_update_from(activity);
		}
	}
}

void ActivityCore::attach_interaction_observer(ActivityObserverInterface *observer) {
	std::unique_lock<std::mutex> lock(interaction_observers_mutex);
	interaction_observers.push_back(observer);
}

void ActivityCore::detach_interaction_observer(ActivityObserverInterface *observer) {
	std::unique_lock<std::mutex> lock(interaction_observers_mutex);
	interaction_observers.remove(observer);
}

int ActivityCore::execute_protected_region()
{
...
void ActivityCore::updateAllCommObjects()
{
}



void ActivityCore::triggerLogEntry(const int& idOffset)
{
	if(useLogging == true) {
...
int ActivityCore::getPreviousCommObjId()
{
