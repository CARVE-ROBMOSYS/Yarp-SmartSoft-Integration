...
SmartACE::CommParameterResponse ParamUpdateHandler::handleParameter(const SmartACE::CommParameterRequest& request)
{
	...
	else if (tag == "COMMYARP_BT.CONFIGPARAMS.CONFIGFILE")
	{
		...
		if(answer.getResponse() == SmartACE::ParamResponseType::OK) {
			triggerHandler.handleCommYARP_BT_ConfigParams_configFileCore(
			temp_filename
			);
		}
	}
...
void ParamUpdateHandler::loadParameter(SmartACE::SmartIniParameter &parameter)
{
	...
	try
	{
		...
		//
		// parameter Settings
		if(parameter.getString("Settings", "configFile", commitState.Settings.configFile))
		{
			globalState.Settings.configFile = commitState.Settings.configFile;
		}
		if(parameter.getString("Settings", "tickManager_localPort", commitState.Settings.tickManager_localPort))
		{
			globalState.Settings.tickManager_localPort = commitState.Settings.tickManager_localPort;
		}
