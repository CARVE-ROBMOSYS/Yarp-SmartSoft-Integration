...
void ThreadCore::notify_all_interaction_observers() {
	std::unique_lock<std::mutex> lock(interaction_observers_mutex);
	// try dynamically down-casting this class to the derived class 
	// (we can do it safely here as we exactly know the derived class)
	if(const Thread* thread = dynamic_cast<const Thread*>(this)) {
		for(auto it=interaction_observers.begin(); it!=interaction_observers.end(); it++) {
			(*it)->on_update_from(thread);
		}
	}
}

void ThreadCore::attach_interaction_observer(ThreadObserverInterface *observer) {
	std::unique_lock<std::mutex> lock(interaction_observers_mutex);
	interaction_observers.push_back(observer);
}

void ThreadCore::detach_interaction_observer(ThreadObserverInterface *observer) {
	std::unique_lock<std::mutex> lock(interaction_observers_mutex);
	interaction_observers.remove(observer);
}

int ThreadCore::execute_protected_region()
{
...
void ThreadCore::updateAllCommObjects()
{
}



void ThreadCore::triggerLogEntry(const int& idOffset)
{
	if(useLogging == true) {
...
int ThreadCore::getPreviousCommObjId()
{
